#include "png_bundle.h"

void PNGBundleBuilder::MemBuffer::read(uint8_t *dst, uint64_t size, uint64_t n) {
    for (uint64_t i = 0; i < size * n; i++) {
        dst[i] = buffer[index++];
    }
}

void PNGBundleBuilder::MemBuffer::write(uint8_t *dst, uint64_t size, uint64_t n) {
    for (uint64_t i = 0; i < size * n; i++) {
        buffer[index++] = dst[i];
    }
}

void PNGBundleBuilder::png_read_fn(png_structp png_ptr, png_bytep data, png_size_t length) {
    auto mem_buf = static_cast<MemBuffer *>(png_get_io_ptr(png_ptr));
    mem_buf->read(data, 1, length);
}

void PNGBundleBuilder::png_vector_write_fn(png_structp png_ptr, png_bytep data, png_size_t length) {
...
void PNGBundleBuilder::png_nop_flush_fn(png_structp png_ptr) {

}

void PNGBundleBuilder::png_error_fn(png_structp png_ptr, png_const_charp error_msg) {
    throw TransformErrors::InvalidOperationError(std::string("libpng: ") + error_msg);
}

void PNGBundleBuilder::png_warning_fn(png_structp png_ptr, png_const_charp warning_msg) {
    LOG_F(WARNING, "libpng: %s", warning_msg);
}

png_voidp PNGBundleBuilder::png_malloc_fn(png_structp png_ptr, png_size_t size) {
    return rainman::Allocator().rmalloc<uint8_t>(size);
}

void PNGBundleBuilder::png_free_fn(png_structp png_ptr, png_voidp ptr) {
    rainman::Allocator().rfree(static_cast<uint8_t *>(ptr));
}

PNGBundleBuilder::PNGBundleBuilder(const rainman::ptr<uint8_t> &buf) {
    _mem_buf.buffer = buf;
}

PNGBundle PNGBundleBuilder::read_bundle() {
    char header[8];
...
rainman::ptr<uint8_t> PNGBundleBuilder::write_pixels(const PNGBundle &bundle) {
